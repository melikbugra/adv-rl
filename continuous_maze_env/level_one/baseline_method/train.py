...
class TrainingConfig:
    ...
    level_name: str = "level_one"
    max_steps: int = 100
    outer_iterations: int = 756
...
class AdvSacTrainer:
    ...
    def _ensure_model_dirs() -> None:
        ...
        for directory in ("models", "level_one_models"):
            os.makedirs(directory, exist_ok=True)
    ...
    def train(self) -> None:
        ...
        for iteration_idx in iteration_bar:
            ...
            if iteration_idx % self.config.checkpoint_period == 0:
                self.adversary.save(
                    save_path=f"level_one_models/adversary_sac_{iteration_idx}"
                )
                self.protagonist.save(
                    save_path=f"level_one_models/protagonist_sac_{iteration_idx}"
                )
