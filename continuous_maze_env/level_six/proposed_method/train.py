...
class TrainingConfig:
    ...
    num_q_heads: int = 6
    sigma_samples: int = 8
    base_lambda_v: float = 1.1
    base_lambda_sigma: float = 0.35
    value_ramp_fraction: float = 0.15
    sigma_warmup_fraction: float = 0.35
    sigma_decay_fraction: float = 0.40
    sigma_min_fraction: float = 0.15
...
class AdvSacTrainer:
    ...
    def _ensure_model_dirs() -> None:
        ...
        for directory in ("models", "level_six_models"):
            os.makedirs(directory, exist_ok=True)
    ...
    def train(self) -> None:
        ...
        for iteration_idx in iteration_bar:
            ...
            if iteration_idx % self.config.checkpoint_period == 0:
                self.adversary.save(
                    save_path=f"level_six_models/adversary_sac_{iteration_idx}"
                )
                self.protagonist.save(
                    save_path=f"level_six_models/protagonist_sac_{iteration_idx}"
                )
