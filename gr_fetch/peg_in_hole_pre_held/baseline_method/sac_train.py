...
Baseline method — identical to proposed_method/sac_train.py to ensure fair
comparison (same architecture, hyperparameters, etc.).
"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="SAC training for FetchPegInHolePreHeldDense-v1 (baseline)"
    )
    parser.add_argument(
        "--timesteps",
        type=int,
    ...
    parser.add_argument(
        "--device", type=str, default=DEVICE, help=f"Torch device (default: {DEVICE})"
    )
    parser.add_argument(
        "--render", action="store_true", help="Render evaluation episodes"
    )
    args = parser.parse_args()
    train(args.timesteps, args.device, args.render)
