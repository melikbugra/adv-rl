"""Check baseline adversarial training progress via MLflow."""
...
ADV_EXP_NAME = (
    "FetchPegInHolePreHeldDense-v1_adv-sac_(baseline,_value-only_reward,_peg-in-hole)"
)
...
def main():
    ...
    for e in exps:
        if "baseline" in e.name.lower() and "peg" in e.name.lower():
            print(f"\nExperiment: {e.name}")
            ...
            for r in runs:
                ...
                if iteration is not None:
                    print(
                        f"  Iteration: {int(iteration)} / 1000  ({iteration / 10:.1f}%)"
                    )
