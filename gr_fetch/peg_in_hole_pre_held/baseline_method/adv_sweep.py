"""Checkpoint sweep for FetchPegInHolePreHeldDense-v1 baseline adversarial training.
...
def eval_checkpoint(
    ...
    agg = dict(
        ...
        mean_adv_tilt_deg=float(
            np.nanmean([s["adv_final_tilt_deg"] for s in summaries])
        ),
...
def sweep(
    ...
    if not checkpoints:
        print(
            f"No complete checkpoint pairs found in '{models_dir}' for range [{start}, {end}]."
        )
        return
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Sweep checkpoints for FetchPegInHolePreHeldDense-v1 baseline adversarial training.",
