"""Adversarial evaluation for FetchPegInHolePreHeldDense-v1 (Baseline).
...
def run_episode(
    ...
    summary = dict(
        ...
        adv_final_tilt_deg=np.degrees(adv_tilt_list[-1])
        if adv_tilt_list
        else float("nan"),
        ...
        prt_final_tilt_deg=np.degrees(prt_tilt_list[-1])
        if prt_tilt_list
        else float("nan"),
...
def evaluate(
    ...
    if n > 1:
        ...
        print("=" * len(_HDR))
        print(
            f"  Adversary score      mean={np.mean(adv_scores):+.3f}  std={np.std(adv_scores):.3f}"
        )
        print(
            f"  Protagonist score    mean={np.mean(prt_scores):+.3f}  std={np.std(prt_scores):.3f}"
        )
        print(
            f"  Protagonist success  {sum(successes)}/{n}  ({100 * np.mean(successes):.0f}%)"
        )
        print()
        print(f"  Adv terminal state:")
        print(
            f"    d_xy  mean={np.mean(adv_d_xy):.4f}  (higher = harder for protagonist)"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Baseline adversarial evaluation for FetchPegInHolePreHeldDense-v1",
