"""
SAC training script for FetchStirDense-v1.

Architecture mirrors peg_in_hole_pre_held/sac_train.py — only ENV_ID and
MAX_EPISODE_STEPS differ.
"""
...
def make_env(
    env_id: str, max_episode_steps: int, render_mode: str = None, flatten: bool = False
):
...
def train(timesteps: int, device: str, render_eval: bool):
    ...
    eval_env = make_env(
        ENV_ID, MAX_EPISODE_STEPS, render_mode=eval_render, flatten=True
    )
    ...
    model = SAC(
        ...
        tau=TAU,
        gamma=GAMMA,
        target_entropy=-float(action_dim) / 4,   # -1.0 → daha keşifçi
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SAC training for FetchStirDense-v1")
    parser.add_argument(
        "--time-steps",
        "--timesteps",
        dest="timesteps",
    ...
    parser.add_argument(
        "--render", action="store_true", help="Render eval env during training"
    )
    parser.add_argument(
        "--eval-only", action="store_true", help="Evaluate best_avg checkpoint only"
    )
